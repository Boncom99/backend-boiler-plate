...
export const userBookFactory = async (
  prisma,
  userBook: Partial<UserBook>,
  book: Book,
  numberOfReadingSessions?: number,
) => {
  ...
  for (
    let i = 0;
    i < numberOfReadingSessions || faker.number.int({ min: 2, max: 10 });
    i++
  ) {
    const startPage = lastPage;
    const endPage = Math.min(
      startPage + faker.number.int({ min: 1, max: 20 }),
      book.pages,
    );
